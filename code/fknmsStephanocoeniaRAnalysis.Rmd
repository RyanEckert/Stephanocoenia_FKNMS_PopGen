...
floridaMap = ggplot() +
  ...
  geom_point(data = fknmsSites, aes(x = longDD, y = latDD, shape = depth), size = 0) +
...
sintMat$sample.a = factor(row.names(sintMat), levels = unique(sampleOrder))
...
relateHeatmapA = ggplot(data = sintFam, aes(sample.a, sample.b, fill = relate)) +
  ...
  scale_color_manual(breaks = legend_ord, values = c("Region" = "white", "Upper Keys" = "#28ADA8", "Lower Keys" = "#3F86BC", "Tortugas Bank" = "#7A3A9A", "Riley's Hump"  = "#392682", " " = "white", "Depth Zone" = "white", "Shallow" = "gray75", "Mesophotic" = "gray35"), name = NULL, guide = guide_legend(override.aes = list(size = 7), order = 2)) +
...
```{r, mAF2}
mafDensPlotA = ggplot(data = fkSintAF, aes(x = minFreq, group = pop, fill = NA, linetype = depthZone, color = region)) + 
...
```{r, inbredMiAF2}
mafDensInbredPlotA = ggplot(data = fkSintInbredAF, aes(x = minFreq, group = pop, fill = NA, linetype = inbred, color = region)) + 
  geom_density(alpha = 0.5, size = 0.25) +
  xlab("Outlier locus minor allele frequency") +
  ylab("Density") +
  scale_color_manual(values = flPal, name = "Region") +
...
ggsave("../figures/admixturePlot.png", plot = admixPlot, width = 22, height = 8, units = "cm", dpi = 300)
ggsave("../figures/admixturePlot.eps", plot = admixPlot, width = 22, height = 8, units = "cm", dpi = 300)
